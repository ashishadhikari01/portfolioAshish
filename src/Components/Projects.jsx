...
export default function Projects(props){
 ...
 let details=projectData.map((data)=>{
   ...
   <p><span>Github:</span><a href={data.github} style={{color:props.whiteBackground? "darkblue":"darkcyan"}}target="_blank">{data.github}</a></p>
   <p><span>Live:</span><a href={data.live} target="_self" style={{color:props.whiteBackground? "darkred":"darkgoldenrod"}}>{data.live}</a></p>
   <p className="technologyUsed">{data.technologyUsed.map((item)=>{
     return <span style={{color:props.whiteBackground? "black":"white"}}>{item}</span>
   })}</p> 
      </div>
  </div>
   )
